...
La figura se conoce como *lineas de regresion muestral*. Ya suponemos que representan la linea de regresion poblacional, pero, debido a fluctuaciones muestrales, son, en el mejor de los casos, solo una aproximacion del averdadera *RP*.

Esto desarrollara el concepto de *funcion de regresion muestras* (FRM) para representar la linea de regresion muestral.

$$\hat{Y_{i}} = \hat{\beta_{1}} + \hat{\beta_{2}}X_{i}$$

donde $\hat{Y}$ se lee "/Y de sombrero/" o "/Y gorra/"

- $\hat{Y_{i}}$: estimador de $E(Y|X_{i})$
- $\hat{\beta_{1}}$: estimador de $\beta_{}_{i}$
- $\hat{\beta_{2}}$: estimador de $\beta_{2}$

Recordar que un *estimador*, conocido tambien concido como *estadistico* (muestral), no es mas que una regla.

Cabe señalar que un estimador es aleatorio, pero una estimacion no.

Para entenderlo veamos como se expresa en su forma estocastica:

$$Y_{i} = \hat{\beta_{1}} + \hat{\beta_{2}}X_{i} + \hat{u_{i}}$$

donde, ademas de los simbolos ya definidos, $\hat{u_{i}_{}}$ denota el termino *residual* (muestral). Conceptualmmente, $\hat{u_{i}}$ es analogo a $u_{i}$ y se Conceptualmmente, $\hat{u_{i}} es analogo a $u_i$ y se considera una /estimacion/ de $u_i$, que introduce en la FRP.

Asi, para resumir, concluimos que le objetico principal del analisis de regresion es estimar la FRP

$$Y_{i} = \beta_{1} + \beta_{2}X_{i} + u_{i}$$

Con base en la FRM

$$Y_{i} = \hat{\beta_{1}} + \hat{\beta_{2}_{}}x_{i }+ \hat{u_{i}}$$

porque son mas frecuentes los casos en que el analisis se basa en una sola muestra tomada de una poblacion. Pero debido a fluctuaciones muestrales, la estimacion de la FRP basada en la FRM es, en el mejor de los caso, una aproximacion.


#+CAPTION: Figura 2.5 Lineas de regresion muestral y poblacional
[[file:graficos/Figura-2_5.png]]
---
