...
\begin{equation}
\begin{split}
  \ln Y_t = \beta_0 & + \beta_1 \ln X_{2t} + \beta_2 \ln X_{3t} + \beta_3 \ln X_{4t} + \beta_4 \ln X_{5t} \\
                    & + \delta_0 D_t + \delta_1 D_t \ln X_{2t} + \delta_2 D_t \ln X_{3t} \\
                    & + \delta_3 D_t \ln X_{4t} + \delta_4 D_t \ln X_{5t} + u_t
\end{split}
\label{eq:chow}
\end{equation}
...
\small
\begin{tabular}{p{1.2cm}p{4.5cm}p{4.5cm}c} % Definición de anchos fijos para evitar que se corte
\toprule
\textbf{At.} & \textbf{Criterio} & \textbf{Resultado} & \textbf{Veredicto} \\
\midrule
1 & Admisibilidad de datos (predicciones $> 0$) & Forma log-log garantiza $\hat{Y} > 0$ & \checkmark \\
2 & Consistencia con la teoría económica & Signos correctos; sustitutos insignificantes & $\sim$ \\
3 & Exogeneidad débil de los regresores & Simultaneidad precio-cantidad; MCO sesgado & $\times$ \\
4 & Constancia de los parámetros & Prueba de Chow: $F(5,13)=3.93, p=0.022$ & $\times$ \\
5 & Coherencia de datos (no autocorrelación) & $d=1.826$; BG: $p=0.605/0.291$ & \checkmark \\
6 & Capacidad de inclusión & No evaluada en el texto original & --- \\
\bottomrule
\end{tabular}
\end{center}
